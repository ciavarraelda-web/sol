...
// Funziona con il frontend Streamlit già fornito
// Pronto da copiare: collega il mining reale su Solana
...
// Puoi mettere la chiave privata del wallet sender come variabile env: APP_WALLET_SECRET (base58)
const APP_WALLET_SECRET = process.env.APP_WALLET_SECRET; // base58
if (!APP_WALLET_SECRET) {
  console.error("Missing APP_WALLET_SECRET env variable!");
  process.exit(1);
}
const APP_WALLET = Keypair.fromSecretKey(Uint8Array.from(require("bs58").decode(APP_WALLET_SECRET)));
...
// Birdeye API per prezzo (puoi usare CoinGecko se vuoi)
...
app.post("/api/mine", async (req, res) => {
  ...
  // Invio token SPL
  try {
    // Ottieni account token destinatario
